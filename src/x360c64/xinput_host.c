...
// Wired 360 commands
static const uint8_t xbox360_wired_rumble[] = {0x00, 0x08, 0x00, 0x00,
                                               0x00, 0x00, 0x00, 0x00};
...
static const uint8_t xboxone_power_on[] = {GIP_CMD_POWER, GIP_OPT_INTERNAL,
                                           GIP_SEQ0, GIP_PL_LEN(1), GIP_PWR_ON};
static const uint8_t xboxone_s_init[] = {GIP_CMD_POWER, GIP_OPT_INTERNAL,
                                         GIP_SEQ0,      GIP_PL_LEN(15),
                                         0x06,          0x00,
                                         0x00,          0x00,
                                         0x00,          0x00,
                                         0x00,          0x55,
                                         ...
                                         0x00,          0x00,
                                         0x00};
static const uint8_t xboxone_s_led_init[] = {
    GIP_CMD_LED, GIP_OPT_INTERNAL, GIP_SEQ0, GIP_PL_LEN(3), 0x00, 0x01, 0x14};
static const uint8_t extra_input_packet_init[] = {0x4d, 0x10, GIP_SEQ0,
                                                  0x02, 0x07, 0x00};
static const uint8_t xboxone_pdp_led_on[] = {
    GIP_CMD_LED, GIP_OPT_INTERNAL, GIP_SEQ0, GIP_PL_LEN(3),
    0x00,        GIP_LED_ON,       0x14};
static const uint8_t xboxone_pdp_auth[] = {GIP_CMD_AUTHENTICATE,
                                           GIP_OPT_INTERNAL,
                                           GIP_SEQ0,
                                           GIP_PL_LEN(2),
                                           0x01,
                                           0x00};
static const uint8_t xboxone_rumble[] = {
    GIP_CMD_RUMBLE, 0x00, 0x00, GIP_PL_LEN(9), 0x00, GIP_MOTOR_ALL, 0x00,
    0x00,           0x00, 0x00, 0xFF,          0x00, 0xFF};

// Wireless 360 commands
static const uint8_t xbox360w_led[] = {0x00, 0x00, 0x08, 0x40};
// Sending 0x00, 0x00, 0x08, 0x00 will permanently disable rumble until you do
// this:
...
TU_ATTR_ALWAYS_INLINE static inline xinputh_device_t *get_dev(
    uint8_t dev_addr) {
  return &_xinputh_dev[dev_addr - 1];
}

TU_ATTR_ALWAYS_INLINE static inline xinputh_interface_t *get_instance(
...
static void xboxone_init(xinputh_interface_t *xid_itf, uint8_t dev_addr,
                         uint8_t instance) {
  uint16_t PID, VID;
  tuh_vid_pid_get(dev_addr, &VID, &PID);

  tuh_xinput_send_report(dev_addr, instance, xboxone_power_on,
                         sizeof(xboxone_power_on));
  wait_for_tx_complete(dev_addr, xid_itf->ep_out);
  tuh_xinput_send_report(dev_addr, instance, xboxone_s_init,
                         sizeof(xboxone_s_init));
  wait_for_tx_complete(dev_addr, xid_itf->ep_out);

  if (VID == 0x045e && (PID == 0x0b00)) {
    tuh_xinput_send_report(dev_addr, instance, extra_input_packet_init,
                           sizeof(extra_input_packet_init));
    wait_for_tx_complete(dev_addr, xid_itf->ep_out);
  }

  // Required for PDP aftermarket controllers
  if (VID == 0x0e6f) {
    tuh_xinput_send_report(dev_addr, instance, xboxone_pdp_led_on,
                           sizeof(xboxone_pdp_led_on));
    wait_for_tx_complete(dev_addr, xid_itf->ep_out);
    tuh_xinput_send_report(dev_addr, instance, xboxone_pdp_auth,
                           sizeof(xboxone_pdp_auth));
    wait_for_tx_complete(dev_addr, xid_itf->ep_out);
  }
}
...
bool tuh_xinput_receive_report(uint8_t dev_addr, uint8_t instance) {
  ...
  if (!usbh_edpt_xfer(dev_addr, xid_itf->ep_in, xid_itf->epin_buf,
                      xid_itf->epin_size)) {
...
bool tuh_xinput_set_led(uint8_t dev_addr, uint8_t instance, uint8_t quadrant,
                        bool block) {
  xinputh_interface_t *xid_itf = get_instance(dev_addr, instance);
  uint8_t txbuf[32];
  uint16_t len;
...
bool tuh_xinput_set_rumble(uint8_t dev_addr, uint8_t instance, uint8_t lValue,
                           uint8_t rValue, bool block) {
  xinputh_interface_t *xid_itf = get_instance(dev_addr, instance);
  uint8_t txbuf[32];
  uint16_t len;
...
// USBH API
//--------------------------------------------------------------------+
void xinputh_init(void) { tu_memclr(_xinputh_dev, sizeof(_xinputh_dev)); }

bool xinputh_open(uint8_t rhport, uint8_t dev_addr,
  ...
  if (desc_itf->bNumEndpoints < 2)
    type = XINPUT_UNKNOWN;
  else if (desc_itf->bInterfaceSubClass ==
               0x5D &&  // Xbox360 wireless bInterfaceSubClass
           desc_itf->bInterfaceProtocol ==
               0x81)  // Xbox360 wireless bInterfaceProtocol
    type = XBOX360_WIRELESS;
  else if (desc_itf->bInterfaceSubClass ==
               0x5D &&  // Xbox360 wired bInterfaceSubClass
           desc_itf->bInterfaceProtocol ==
               0x01)  // Xbox360 wired bInterfaceProtocol
    type = XBOX360_WIRED;
  else if (desc_itf->bInterfaceSubClass ==
               0x47 &&  // Xbone and SX bInterfaceSubClass
           desc_itf->bInterfaceProtocol ==
...
bool xinputh_set_config(uint8_t dev_addr, uint8_t itf_num) {
  ...
  if (xid_itf->type == XBOX360_WIRELESS) {
    ...
    tuh_xinput_send_report(dev_addr, instance, xbox360w_inquire_present,
...
bool xinputh_xfer_cb(uint8_t dev_addr, uint8_t ep_addr, xfer_result_t result,
  ...
  if (result != XFER_RESULT_SUCCESS) {
    if (dir == TUSB_DIR_IN) {
      tuh_xinput_report_received_cb(dev_addr, instance, xid_itf,
                                    sizeof(xinputh_interface_t));
    } else if (tuh_xinput_report_sent_cb) {
      tuh_xinput_report_sent_cb(dev_addr, instance, xid_itf->epout_buf,
                                xferred_bytes);
    }
    return false;
  }
